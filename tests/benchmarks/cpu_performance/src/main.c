 ...
 * @c CONFIG_ARM64_PMU is enabled) and measures CPU behavior on Cortex-A
 * class processors.
 *
 * Test categories:
...
ZTEST(cpu_performance, test_05_memory_bandwidth)
{
	...
	cpu_mhz = pmu_cpu_freq_mhz();
	if (cpu_mhz > 0 && cycles > 0) {
		double bytes_accessed = LARGE_ARRAY_ELEMENTS * sizeof(uint64_t);
		double bandwidth_mbps = (bytes_accessed /
					 (double)cycles) * cpu_mhz;
...
static void *cpu_perf_setup(void)
{
	 ...
	 * preempted or migrated to another CPU mid-measurement.  PMU
	 * registers are per-CPU, but cooperative scheduling guarantees
	 * affinity without explicit pinning of the running thread.
	 */
	test_setup();
	return NULL;
}
